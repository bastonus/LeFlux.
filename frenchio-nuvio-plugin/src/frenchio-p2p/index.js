/**
 * LeFlux. P2P — Nuvio Plugin Provider
 *
 * Nuvio appelle : getStreams(tmdbId, mediaType, season, episode)
 *   - tmdbId    : ID TMDB du film/série (ex: "12345")
 *   - mediaType : "movie" ou "tv"
 *   - season    : numéro de saison (null pour films)
 *   - episode   : numéro d'épisode (null pour films)
 *
 * Les clés des trackers sont stockées en settings dans Nuvio.
 * On les lit depuis globalThis._frenchioSettings (injecté par Nuvio).
 */

// ── Settings ─────────────────────────────────────────────────────────────────
// Nuvio injecte les settings via le champ "settings" du manifest.
// Pour tester manuellement, vous pouvez pré-remplir ces valeurs ici.
...
function buildMagnet(hash, name, trackers) {
  ...
  var trs = (trackers || []).map(function(t) { return '&tr=' + encodeURIComponent(t); }).join('');
  return base + trs;
}

// Standard open trackers for announces
var OPEN_TRACKERS = [
...
function formatStreamCard(torrent, trackerName) {
  ...
  // 7. Assemble Name and Title
  var nameParts = [];
  if (q) nameParts.push(q);
  if (szLabel) nameParts.push(szLabel);
  if (langBadge) nameParts.push(langBadge);
  nameParts.push(trackerName);
  var formattedName = nameParts.join(' · ');

  var descParts = [];
  if (releaseBadge) descParts.push(releaseBadge);
  if (codecBadge) descParts.push(codecBadge);
  if (hdrBadge) descParts.push(hdrBadge);
  if (audioBadge) descParts.push(audioBadge);
  var formattedDesc = descParts.join(' · ');
...
// resolveMagnetTrackers removed (using getStreamUrl instead)
...
function titleMatch(torrentName, expectedTitle) {
  ...
  return n.includes(t) || t.split(' ').every(function(w) { return w.length < 3 || n.includes(w); });
}
...
function seasonEpisodeMatch(torrentName, season, episode) {
  ...
  // Cherche S01 ou S01E01
  var hasS = n.includes('S' + s);
  if (!hasS) return false;
  if (!e) return true; // saison seule ok
  return n.includes('S' + s + 'E' + e) || n.includes('E' + e);
}

// ── Formatage stream Nuvio ────────────────────────────────────────────────────
...
function formatStream(torrent, trackerName, streamUrl) {
  ...
  if (maxSz > 0 && size > maxSz) return null;

  var hash = (torrent.info_hash || torrent.hash || '').toLowerCase();
  
  // Re-build trackers list for Stremio/Nuvio native torrent engine sources
  ...
  return {
    ...
    url:         streamUrl,
    infoHash:    hash,
    sources:     trackers.map(function(t) { return 'tracker:' + t; }),
...
// ── TMDB title and external IDs resolution ─────────────────────────────────────
  ...
  return safeFetch(url)
    ...
    .then(function(d) {
      ...
      if (dateStr) {
        ...
        if (parts[0]) year = parseInt(parts[0]) || null;
      }
      return { title: title, imdbId: imdbId, year: year };
    })
    .catch(function() {
...
function queryC411(mediaInfo, tmdbId, mediaType, season, episode) {
  ...
  var searchUnit = function(ep) {
    ...
    if (mediaType === 'movie') {
      ...
      if (ep !== null && ep !== undefined) params.set('episode', ep);
    }
      ...
      .then(function(data) {
        ...
        return items.map(function(item) {
          ...
          for (var i = 0; i < attrs.length; i++) {
            ...
            if (attr.name === 'infohash') info_hash = attr.value;
            if (attr.name === 'seeders') seeders = parseInt(attr.value) || 0;
          }
          if (!info_hash) info_hash = item.guid;
  ...
  var searchUnit = function(ep) {
    ...
    return safeFetch(url)
      ...
      .then(function(xml) {
        ...
        var filtered = items;
        if (mediaInfo.title) {
          filtered = items.filter(function(t) { return titleMatch(t.name, mediaInfo.title); });
        }
        if (mediaType === 'tv' && season) {
          filtered = filtered.filter(function(t) { return seasonEpisodeMatch(t.name, season, ep); });
        }
...
function parseTorznabItems(xml) {
  ...
  while ((m = itemRe.exec(xml)) !== null) {
    ...
    var hash = getAttr('torznab:attr[name="infohash"]', 'value') ||
               getAttr('torznab:attr', 'value');
    var infohashMatch = block.match(/name="infohash"\s+value="([^"]+)"/i);
    if (infohashMatch) hash = infohashMatch[1];

    var sizeStr = get('size') || getAttr('enclosure', 'length');
    var seeders = 0;
    var seedersMatch = block.match(/name="seeders"\s+value="([^"]+)"/i);
    if (seedersMatch) seeders = parseInt(seedersMatch[1]) || 0;
...
function searchTR4KER(title, mediaType, season, episode) {
  ...
  if (!apikey || !title) return Promise.resolve([]);

  var query = title;
  if (season) query += ' S' + String(season).padStart(2, '0');
  if (episode) query += 'E' + String(episode).padStart(2, '0');
  ...
  return safeFetch(url)
    ...
    .then(function(xml) {
      var items = parseTorznabItems(xml);
      return items
        .filter(function(t) { return titleMatch(t.name, title); })
        .filter(function(t) { return seasonEpisodeMatch(t.name, season, episode); })
        .map(function(t) {
          return {
            info_hash: t.info_hash,
            name:      t.name,
...
function sortStreams(torrents) {
  return torrents.sort(function(a, b) {
    var qa = QUALITY_ORDER[qualityFromName(a.name || a.title || '')] !== undefined ? QUALITY_ORDER[qualityFromName(a.name || a.title || '')] : 99;
    var qb = QUALITY_ORDER[qualityFromName(b.name || b.title || '')] !== undefined ? QUALITY_ORDER[qualityFromName(b.name || b.title || '')] : 99;
    return qa - qb;
  });
}
...
function getStreams(tmdbId, mediaType, season, episode) {
  ...
  var settings = getSettings();
  var hasAnyKey = settings.c411_apikey || settings.torr9_passkey ||
                  settings.tr4ker_apikey || settings.gemini_apikey;
  ...
  return resolveMediaInfoFromTmdb(tmdbId, mediaType)
    ...
    .then(function(results) {
      ...
      var formatted = top15.map(function(t) {
        ...
        if (t.source === 'c411') trackerName = 'C411';
        else if (t.source === 'torr9') trackerName = 'Torr9';
        else if (t.source === 'gemini') trackerName = 'Gemini';
        else if (t.source === 'tr4ker') trackerName = 'TR4KER';
        
        // Adjust size for episode if needed
        var size = t.size || 0;
        if (mediaType === 'tv' && season !== null && season !== undefined && episode !== null && episode !== undefined) {
        ...
        var torrentCopy = {};
        for (var k in t) { torrentCopy[k] = t[k]; }
